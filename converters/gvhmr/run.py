...
# ==================== SMPL-X 维度硬修复猴子补丁 ====================
# 强行拦截 torch.einsum，当发现是 SMPL-X 蒙皮矩阵爆炸时进行对齐
original_einsum = torch.einsum

def patched_einsum(equation, *operands):
    if equation == 'bl,mkl->bmk' and len(operands) == 1:
        # operands 在高版本 torch 中可能是一个 tuple
        ops = operands[0]
        betas, shape_disps = ops[0], ops[1]
        
        l_betas = betas.shape[-1]        # 通常是 20
        l_disps = shape_disps.shape[-1]  # 通常是 26
        
        if l_betas != l_disps:
            if l_betas < l_disps:
                # 情况 A：betas 只有 20 维，而资产需要 26 维 -> 后面用 0 补齐 6 维
                padding = torch.zeros((*betas.shape[:-1], l_disps - l_betas), dtype=betas.dtype, device=betas.device)
                betas = torch.cat([betas, padding], dim=-1)
            else:
                # 情况 B：betas 太长 -> 直接截断到符合资产的维度
                betas = betas[..., :l_disps]
            
            # 重新打包写回 operands
            ops = (betas, shape_disps)
        return original_einsum(equation, ops)
