...
class SOMASkeleton77(SkeletonBase):
    ...
    def to_SMPLXSkeleton22(self, local_rot_mats: torch.Tensor, root_positions: torch.Tensor):
        """Retarget 77-joint SOMA motion to 22-joint SMPL-X skeleton.

        Uses global-rotation mapping: compute SOMA77 global rots, copy matched
        joints' global rots into SMPLX22 slots, then derive SMPLX22 local rots.
        """
        from kimodo.skeleton.transforms import global_rots_to_local_rots

        target = SMPLXSkeleton22()
        T = local_rot_mats.shape[0]
        device = local_rot_mats.device
        dtype = local_rot_mats.dtype

        # Step 1: SOMA77 FK -> global rotations
        soma_global, _, _ = self.fk(local_rot_mats, root_positions)

        # Step 2: Build SMPLX22 global rotations from mapping
        smplx_global = torch.zeros((T, 22, 3, 3), device=device, dtype=dtype)
        for soma_name, smplx_name in _SOMA77_TO_SMPLX22_MAP.items():
            si = self.bone_index[soma_name]
            ti = target.bone_index[smplx_name]
            smplx_global[:, ti] = soma_global[:, si]

        # Step 3: Global -> local for SMPLX22 hierarchy
        smplx_local = global_rots_to_local_rots(smplx_global, target)

        return smplx_local, target
